...
int main(int argc, const char *argv[])
{
    ...
    vector<double> ttcCameraResults;
    vector<double> ttcLidarResults;
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        {
            cout << "Bounding box " << it - (dataBuffer.end() - 1)->boundingBoxes.begin() << " has lidar points: " << it->lidarPoints.size() << endl;
        }
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    ttcLidarResults.push_back(ttcLidar);
                    ttcCameraResults.push_back(ttcCamera);
    ...
    string ttcFileName = "ttc_results_" + detectorType + "_" + descriptorType + ".csv";

    ofstream ttcFile(ttcFileName);
    if (ttcFile.is_open())
    {
        // Write the header
        ttcFile << "Frame, TTC Camera, TTC Lidar" << endl;
        
        // Write the TTC values for each frame
        for (size_t i = 0; i < ttcCameraResults.size(); ++i)
        {
            ttcFile << i << ", " << ttcCameraResults[i] << ", " << ttcLidarResults[i] << endl;
        }
